...
const CareerChapterReader: React.FC = () => {
  ...
  const next = () => current < careerChapters.length - 1 && setCurrent(current + 1);
  ...
  return (
    <div className="w-full h-screen py-6 px-4 md:px-6 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto pb-4">
        <h2 className="text-2xl md:text-3xl font-bold mb-4 text-center">{careerChapters[current].title}</h2>
        {careerChapters[current].content.split("\n\n").map((para, i) => (
          <p key={i} className="mb-4 text-base md:text-lg leading-relaxed whitespace-pre-wrap">{para}</p>
        ))}
      </div>
